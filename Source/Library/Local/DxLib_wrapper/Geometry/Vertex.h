...
struct IndexedVertex2D {
	...
	IndexedVertex2D Framed(float thickness = 1, size_t rIgnoreCount = 0) {
		...
		for(size_t i = 0,c = polygonnum / 2; i < c; ++i) {
			...
			ret.index[(i * 6) + 4] = (i * 2 + 0) % polygonnum;
			ret.index[(i * 6) + 5] = (i * 2 + 2) % polygonnum;
		}

		return ret;
	}

	IndexedVertex2D TrussFramed(float thickness = 1) {
		IndexedVertex2D ret;
		float half = thickness / 2;
		...
		for (size_t i = 0, count = polygonnum / 3; i < count; ++i) {
			auto &&v1 = this->vertex[this->index[i]];
			...
			ret.vertex[first + 2].pos = v1.pos + c2;
			ret.vertex[second + 2].pos = v1.pos - c2;
			
			ret.vertex[first + 2].color = v1.color;
			ret.vertex[second + 2].color = v1.color;
			
			ret.vertex[first + 4].pos = v3.pos + c3;
			ret.vertex[second + 4].pos = v3.pos - c3;
	...
	Val2D<float> __FramedImpl(const Val2D<float> &p1, const Val2D<float> &p2, const Val2D<float> &p3) {
		Val2D<float> d1 = (p1 - p2).Normalized(), d2 = (p3 - p2).Normalized();
		Val2D<float> n1 = Val2D{-d1.y, d1.x}, n2 = Val2D{-d2.y, d2.x};
...
namespace Vertex {
	...
	namespace Factory2D {
		...
		static IndexedVertex2D Line(const Val2D<float> &p1, const Val2D<float> &p2, const Color4 &color, float thickness = 1.f) {
			...
			float half = thickness / 2;
			const Val2D<float> norm = (p2 - p1).Normalized();
			const Val2D<float> d = norm.Rotate(pi / 2) * half;
			ret.vertex[0].pos = p1 + -d;
			ret.vertex[1].pos = p2 + -d;
			ret.vertex[2].pos = p2 + d;
			ret.vertex[3].pos = p1 + d;
